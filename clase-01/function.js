...
console.log(
	posts.reduce(
		function(acc,post2){
            if(!acc[post2.userId]) {
        //    if( typeof acc[post2.userId] === 'undefined') {
				// console.log('a');
...
// LEER: usa un objeto y al manejarlo como un array dentro del reduce, estoy haciendo
// referencia al nombre del metodo, no al indice.
// en javascript todo es un objeto y cuando pongo obj.algo, .algo es un atributo de obj
// puede ser un metodo como .reduce o hacer referencia a una variable del mismo
...
//  req.send();
// }
// request('https://jsonplaceholder.typicode.com/posts', (response, error) => console.log(response, error));
//
...
llamada('https://jsonplaceholder.typicode.com/posts').then(
    function(result) {
        console.log(result);
    }
).catch(
